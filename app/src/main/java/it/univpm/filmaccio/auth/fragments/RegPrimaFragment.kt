...
class RegPrimaFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        buttonAvanti.setOnClickListener {
            ...
            // Il nome utente viene salvato in minuscolo per evitare problemi di case sensitivity,
            // quindi tutti gli username saranno in minuscolo
            ...
            // Rimozione di eventuali errori precedenti nei textLayout)
            ...
            if (!isUsernameValid(username)) {
                // Se non è valido, viene mostrato un errore nel textLayout e si interrompe l'esecuzione
                // del codice mostrando un messaggio che spiega come deve essere strutturato lo username
            ...
            if (!isPasswordValid(password)) {
                // Se non è valida, viene mostrato un errore nel textLayout e si interrompe l'esecuzione
                // del codice mostrando un messaggio che spiega come deve essere strutturata la password
            ...
            isEmailAlreadyRegistered(email) { emailExists ->
                if (emailExists) {
                    // Se l'email è già associata ad un account esistente, viene mostrato un errore
                    // nel textLayout e si interrompe l'esecuzione del codice
                    emailRegInputTextLayout.isErrorEnabled = true
                    emailRegInputTextLayout.error =
                        "L'indirizzo email è già associato ad un account esistente"
                } else {
                    // Se l'email non è già associata ad un account esistente, viene verificato
                    // che lo username inserito non sia già associato ad un account esistente
                    isUsernameAlreadyRegistered(username) { usernameExists ->
                        if (usernameExists) {
                            // Se lo username è già associato ad un account esistente, viene mostrato
                            // un errore nel textLayout e si interrompe l'esecuzione del codice
                            usernameRegInputTextLayout.isErrorEnabled = true
                            usernameRegInputTextLayout.error = "Il nome utente non è disponibile"
                        } else {
                            // Se lo username non è già associato ad un account esistente, si naviga
                            // al fragment successivo passando i dati inseriti dall'utente
                            // Qui passiamo i dati inseriti dall'utente tramite l'action creata nel
                            // navGraph (file res/navigation/nav_login.xml in cui ho specificato,
                            // per alcune action, quali dati e di che tipo devono essere passati nella navigazione)
                            // In questo caso passiamo tre stringhe: email, username e password
                            // che poi possono essere riottenute nel fragment successivo
                            val action =
                                RegPrimaFragmentDirections.actionRegPrimaFragmentToRegSecondaFragment(
                                    email, username, password
                                )
                            // Qui navighiamo attraverso l'action definita prima al prossimo fragment.
                            // Infatti l'action si riferisce a actionRegPrimaFragmentToRegSecondaFragment
                            // che è l'id dell'action che porta dal framgnet RegPrimaFragment al
                            // fragment RegSecondaFragment mantenendo i dati necessari
                            Navigation.findNavController(binding.root).navigate(action)
                        }
                    }
                }
            }
            // Questo if serve per fare un return@setOnClickListener dopo che si è verificato
            // che l'email è già associata ad un account esistente o che lo username è già associato ad un account esistente
    ...
    // Metodo che si occupa di verificare che l'email inserita sia valida in senso di pattern
    // di email, quindi controlla che sia della forma "qualcosa@qualcosa.qualcosa"
    private fun isEmailValid(email: String): Boolean {
        return Patterns.EMAIL_ADDRESS.matcher(email).matches()
    }

    // Metodo che si occupa di verificare se l'indirizzo email inserito sia già
    // associato ad un account esistente nel database FIRESTORE
    private fun isEmailAlreadyRegistered(email: String, callback: (Boolean) -> Unit) {
        // Quì creiamo la query che andrà a verificare se esiste un documento nel
        // database che abbia un campo email uguale a quello inserito dall'utente
        // Nota: per creare la query usiamo la classe FirestoreService che contiene tutti
        // i metodi per interagire con il database FIRESTORE
        ...
        query.get().addOnSuccessListener { querySnapshot ->
            // Quì verifichiamo che la query abbia restituito dei risultati, se non ha restituito
            // risultati vuol dire che non esiste un documento nel database con l'email inserita dall'utente
            val exists = !querySnapshot.isEmpty
            // Quì chiamiamo la callback passandogli il risultato della query
            // La callback è una funzione che viene passata come parametro ad un'altra
            // funzione e che viene chiamata all'interno di quest'ultima, in altre parole è quel "emailExists"
            // che viene chiamato all'interno della lambda relativa a isEmailAlreadyRegistered dentro il listener del pulsante buttonAvanti
            callback(exists)
        }.addOnFailureListener {
            // Se la query fallisce, probabilmente è dovuto ad un errore di
            // connessione, quindi mostriamo un messaggio di errore
            ...
            // Facciamo così perché essendoci stato un errore di connessione, non possiamo sapere
            // se l'email esiste o meno, quindi per sicurezza assumiamo che non esista e facciamo riprovare l'inserimento all'utente
            callback(true)
        }
    }

    // Metodo che si occupa di verificare che lo username inserito sia valido in senso di
    // pattern di username (scelto da noi), quindi in poche parole controlla che sia della forma giusta
    private fun isUsernameValid(username: String): Boolean {
        // Verifica che il nome utente non contenga caratteri speciali, che abbia una lunghezza minima
        // di 3 caratteri e una lunghezza massima di 30 caratteri, e che gli unici caratteri speciali che può contenere siano _ e .
        // Questo controllo lo fa tramite un pattern regex (scoperta molto interessante)
    ...
    // Metodo che si occupa di verificare se lo username inserito sia già associato ad un account
    // esistente nel database FIRESTORE, è praticamente uguale a isEmailAlreadyRegistered
    ...
    // Metodo che si occupa di verificare che la password inserita
    // sia valida in senso di pattern di password scelto da noi
    private fun isPasswordValid(password: String): Boolean {
        // Anche qui viene usato un pattern regex per verificare che la password abbia almeno 8 caratteri,
        // almeno una lettera maiuscola, una minuscola e almeno un numero
